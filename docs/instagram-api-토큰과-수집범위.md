...
문서로 확정할 수 없어 STEP 6에서 확인해야 한다.

| # | 확인할 것 | 왜 |
|---|---|---|
| 1 | `profile_picture_url`, `name`, `follows_count`가 실제로 오는가 | 문서상 Public 마커 없음 |
| 2 | 릴스에 `media_url`이 오는가, mp4인가 HLS인가 | 출력 포맷 명시 없음 |
| 3 | `children{media_url}` 중첩 확장이 실제로 되는가 | 규격상 지원이나 공식 예시 없음 |
| 4 | 캐러셀 최상위 `media_url`이 비어 오는가 | 폴백 분기 결정 |
| 5 | `media.limit(n)` 최대값 (25/50/100) | 문서에 숫자 없음 |
| 6 | `media.after()` 로 몇 건까지 거슬러 가는가 | 1만 건 상한 적용 여부 |
| 7 | 비공개·개인·없는 계정의 에러 응답 원문 | 오류 처리 설계용 |
