...
export function VenueFilterPanel({ audience, value, resultCount, onChange }: VenueFilterPanelProps) {
  ...
  return <Card padding="none" className={`venue-filter-panel venue-filter-panel--${audience}`}>
    ...
    <fieldset disabled={!locationReady} className={!locationReady ? 'locked' : ''}>
      ...
      <details open className="venue-filter-step"><summary><span>03</span><div><strong>식사·식대</strong><small>메뉴 방식과 1인 식대</small></div><UtensilsCrossed size={16} /></summary><div className="venue-filter-row"><span>식사 형태</span><div className="venue-filter-chips">{venueMealTypes.map((meal) => <button key={meal} className={value.mealTypes.includes(meal) ? 'active' : ''} onClick={() => set('mealTypes', toggle(value.mealTypes, meal))}>{meal}</button>)}</div></div><div className="venue-filter-row"><span>1인 식대</span><div className="venue-filter-chips">{venueMealPriceRanges.map((range) => <button key={range} className={value.mealPriceRanges.includes(range) ? 'active' : ''} onClick={() => set('mealPriceRanges', toggle(value.mealPriceRanges, range))}>{range}</button>)}</div></div></details>
    ...
    <footer className="venue-filter-footer"><label><Search size={16} /><input value={value.query} disabled={!locationReady} onChange={(event) => set('query', event.target.value)} placeholder="홀명·지역·역·터미널 검색" /></label><Button size="sm" variant="ghost" icon={<RotateCcw size={13} />} onClick={() => onChange({ regionGroup: '', localities: [], accessKinds: [], accessPointIds: [], accessOptions: [], mealTypes: [], mealPriceRanges: [], venueTypes: [], wishes: [], query: '' })}>전체 초기화</Button><span><MapPin size={12} /> 같은 단계는 OR, 단계 사이는 AND</span></footer>
  </Card>
}
