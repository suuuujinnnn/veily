export type ReferenceCategory = '드레스' | '헤어' | '메이크업' | '스튜디오' | '웨딩홀';

export interface ReferenceKeywordGroup {
  label: string;
  keywords: string[];
}

export interface ReferenceCategoryDefinition {
  label: ReferenceCategory;
  englishLabel: string;
  description: string;
  searchHint: string;
  groups: ReferenceKeywordGroup[];
}

export const referenceCategories: ReferenceCategoryDefinition[] = [
  {
    label: '드레스',
    englishLabel: 'DRESS',
    description: '라인부터 소재와 장식까지',
    searchHint: '예: 미카도 실크 일자탑, 맑은 비즈',
    groups: [
      // 실측에서 소재를 못 적고 비운 사진이 8장 나왔다. 광택 새틴과 무광 크레이프를
      // 담을 값이 없었기 때문이다. 둘 다 실제 드레스에서 흔한 원단이라 추가한다.
      { label: '소재 1', keywords: ['미카도 실크', '오간자 실크', '튤', '타프타', '새틴', '크레이프'] },
      { label: '소재 2 · 레이스', keywords: ['아플리케 레이스', '블망 레이스', '캐미컬 레이스', '기타 레이스'] },
      // 원래 이름이 '소재 3 · 비즈'였는데 담긴 값은 소재가 아니라 표면 장식이다.
      // 이름을 바로잡고, 실측에서 담을 자리가 없던 값 둘을 넣었다.
      // - 깃털: 이룬 20장 중 2장이 페더 드레스인데 어느 축에도 들어가지 않았다
      // - 장식 없음: 민무늬 드레스와 '판정 불가'가 구분되지 않았다.
      //   인터뷰에서 실제로 쓰인 조건이라 검색어로 살아 있어야 한다
      { label: '장식', keywords: ['글리터·펄', '맑은 비즈', '화려한 비즈', '깃털', '장식 없음'] },
      // 넥라인과 소매를 갈랐다. 한 축일 때는 '오프숄더 + 긴팔소매'처럼 둘 다 참인
      // 드레스에서 한쪽이 버려졌다. 같은 축은 OR, 다른 축은 AND로 묶이기 때문이다.
      {
        label: '넥라인',
        keywords: [
          '일자탑',
          '하트탑',
          '상체 디테일탑',
          '오프숄더',
          '스퀘어넥',
          '하트 스퀘어넥',
          '브이넥',
          '홀터넥',
          '하이넥',
          '라운드넥',
        ],
      },
      { label: '소매', keywords: ['반팔소매', '긴팔소매'] },
      { label: '스커트 라인', keywords: ['A라인', '벨라인', '머메이드라인', '세미라인', '미니멀 기장'] },
      {
        label: '특별 디자인',
        // 오픈백은 등이 파인 것이고 백 포인트는 등에 얹은 장식이다. 한 값으로
        // 뭉뚱그리면 "등이 시원한 드레스"를 찾는 검색이 장식 있는 것만 걸러낸다.
        keywords: ['레이어드 스커트', '캉캉 스커트', '트임 드레스', '미니드레스', '딥 브이', '백 포인트', '오픈백'],
      },
      // 그룹 이름이 용도(촬영·2부)인데 값은 전부 색상이라 본식 화이트를 담을 자리가
      // 애매했다. 값은 그대로 두고 이름만 바꾼다.
      { label: '색상', keywords: ['핑크', '블랙', '블루', '화이트', '퍼플', '그린', '옐로우', '기타'] },
    ],
  },
  {
    label: '헤어',
    englishLabel: 'HAIR',
    description: '단독룸·담당자·시간 조건을 먼저, 디자인은 함께',
    searchHint: '예: 단독룸 원장 지정, 얼리 스타트 로우 번',
    groups: [
      { label: '공간·프라이버시', keywords: ['단독룸', '반독립석', '오픈형'] },
      {
        label: '담당자·진행',
        keywords: ['원장 지정 가능', '실장 지정 가능', '담당자 지정 없음', '1:1 진행', '동시 진행'],
      },
      {
        label: '시간·동선',
        keywords: ['얼리 스타트 가능', '레이트 스타트 가능', '휴무일 진행 가능', '주차·발렛', '출장 가능'],
      },
      {
        label: '추가 서비스',
        keywords: ['본식 헤어피스', '촬영 헤어피스', '혼주 헤어·메이크업', '커트 가능', '헤어 액세서리'],
      },
      { label: '디자인 · 다운', keywords: ['생머리', '웨이브', '단발', '반묶음'] },
      {
        label: '디자인 · 업스타일',
        keywords: ['로우 번', '미들 번', '하이 번', '로우 포니테일', '하이 포니테일', '땋은 머리'],
      },
    ],
  },
  {
    label: '메이크업',
    englishLabel: 'MAKEUP',
    description: '피부 표현과 컬러 무드를 함께',
    searchHint: '예: 투명한 피부, 과즙 메이크업',
    groups: [
      { label: '피부 표현', keywords: ['물광', '세미 매트', '투명', '누드'] },
      { label: '무드·컬러', keywords: ['과즙', '음영', '글램', '강한', '깔끔'] },
    ],
  },
  {
    label: '스튜디오',
    englishLabel: 'STUDIO',
    description: '구도·공간·빛을 조합해서',
    searchHint: '예: 자연광 정원, 인물 중심 클래식',
    groups: [
      { label: '화면 구성', keywords: ['인물 중심', '배경 중심', '인물+배경'] },
      {
        label: '무드·분위기',
        keywords: [
          '모던',
          '클래식',
          '내추럴',
          '따뜻한',
          '감성',
          '사랑스러운',
          '러블리',
          '드라마틱',
          '깔끔',
          '장난스러운',
          '일상',
          '단아한',
          '화보',
          '빈티지',
          '화려한',
          '도시적인',
          '로맨틱',
          '몽환적인',
          '청순한',
          '성숙한',
          '고급스러운',
          '중세',
        ],
      },
      { label: '빛·컬러', keywords: ['자연광', '화이트톤', '선명한', '밝은', '고채도', '싱그러운', '초록', '흑백'] },
      {
        label: '공간·장면',
        keywords: [
          '심플 배경',
          '실내 세트',
          '정원',
          '자연',
          '도시',
          '한옥',
          '분수',
          '아치',
          '저택',
          '롯데타워',
          '루프탑',
          '온실',
          '도로',
          '흑백 씬',
          // 유리 너머 실외가 또렷이 보이는 컷이 4장 있었는데 담을 값이 없어
          // 실내 세트로 뭉갰다. 신부가 실제로 찾는 조건이라 값을 만든다.
          '통창',
        ],
      },
      { label: '시간·구도', keywords: ['낮', '밤', '클로즈업', '독사진', '타이트', '전신', '계란후라이'] },
      { label: '소품', keywords: ['콘셉트 소품', '베일', '생화 부케', '컨페티', '풍선'] },
    ],
  },
  {
    label: '웨딩홀',
    englishLabel: 'WEDDING HALL',
    description: '지역·접근성·식사를 먼저',
    searchHint: '예: 서초구 터미널 뷔페',
    groups: [],
  },
];

export const vendorReferenceKeywords: Record<string, Partial<Record<ReferenceCategory, string[]>>> = {
  'vp-d1': { 드레스: ['A라인', '롱 트레인', '레이스', '볼드한 비즈', '비딩'] },
  'vp-d2': { 드레스: ['벨라인', '롱 트레인', '미카도 실크', '볼드한 비즈'] },
  'vp-d3': { 드레스: ['머메이드', '레이스', '맑고 은은한 비즈', '비딩'] },
  'vp-d4': { 드레스: ['A라인', '모던 미니멀', '숏 트레인', '미카도 실크', '새틴 실크'] },
  'vp-d5': { 드레스: ['프린세스', '롱 트레인', '튤', '레이스', '맑고 은은한 비즈'] },
  'vp-m1': {
    헤어: ['단독룸', '원장 지정 가능', '1:1 진행', '얼리 스타트 가능', '본식 헤어피스', '웨이브', '반묶음', '로우 번'],
    메이크업: ['물광', '투명', '과즙', '깔끔'],
  },
  'vp-m2': {
    헤어: ['반독립석', '실장 지정 가능', '동시 진행', '촬영 헤어피스', '생머리', '하이 번', '하이 포니테일'],
    메이크업: ['물광', '투명', '과즙', '글램'],
  },
  'vp-m3': {
    헤어: ['단독룸', '실장 지정 가능', '1:1 진행', '주차·발렛', '혼주 헤어·메이크업', '웨이브', '반묶음', '미들 번'],
    메이크업: ['세미 매트', '누드', '음영', '깔끔'],
  },
  'vp-m4': {
    헤어: ['오픈형', '담당자 지정 없음', '출장 가능', '커트 가능', '단발', '로우 번', '땋은 머리'],
    메이크업: ['물광', '누드', '과즙', '강한'],
  },
  'vp-m5': {
    헤어: [
      '반독립석',
      '실장 지정 가능',
      '레이트 스타트 가능',
      '휴무일 진행 가능',
      '헤어 액세서리',
      '웨이브',
      '로우 번',
      '로우 포니테일',
    ],
    메이크업: ['투명', '누드', '과즙', '깔끔'],
  },
  'vp-s1': { 스튜디오: ['인물+배경', '화려한', '클래식', '비비드', '실내 세트', '아치', '전신', '콘셉트 소품'] },
  'vp-s2': { 스튜디오: ['인물 중심', '깔끔', '내추럴', '자연광', '화이트톤', '정원', '낮', '클로즈업', '생화 부케'] },
  'vp-s3': { 스튜디오: ['배경 중심', '빈티지', '감성', '따뜻한', '저택', '도로', '밤', '흑백'] },
  'vp-s4': { 스튜디오: ['인물 중심', '모던', '깔끔', '화이트톤', '심플 배경', '실내 세트', '타이트', '베일'] },
  'vp-s5': { 스튜디오: ['인물+배경', '내추럴', '러블리', '자연광', '초록', '정원', '온실', '낮', '전신', '컨페티'] },
};

export function getReferenceCategory(label: ReferenceCategory) {
  return referenceCategories.find((category) => category.label === label) ?? referenceCategories[0];
}
