...
import { ArrowLeft, Check, Heart, ImagePlus, MapPin, Search, Send, Sparkles, Trash2, UploadCloud, WandSparkles } from 'lucide-react'
...
export function ClientTasteDiscovery({ coupleId }: { coupleId: string }) {
  ...
  const [selectedTagsByCategory, setSelectedTagsByCategory] = useState<Partial<Record<ReferenceCategory, string[]>>>({})
  ...
  const venueConditionTags = [venueFilters.regionGroup, ...venueFilters.localities, ...venueFilters.accessKinds, ...venueFilters.accessOptions, ...venueFilters.mealTypes, ...venueFilters.mealPriceRanges, ...venueFilters.venueTypes, ...venueFilters.wishes].filter(Boolean) as string[]
  const setSelectedTags = (next: string[] | ((current: string[]) => string[])) => {
    setSelectedTagsByCategory((all) => ({ ...all, [category]: typeof next === 'function' ? next(all[category] ?? []) : next }))
    setSubmitted(false)
  }
  ...
  const tagSummary = useMemo(() => {
    ...
    const requested = [...Object.values(selectedTagsByCategory).flatMap((tags) => tags ?? []), ...venueConditionTags]
    requested.forEach((tag) => { counts[tag] = (counts[tag] ?? 0) + 100 })
    return Object.entries(counts).sort((a, b) => b[1] - a[1]).map(([tag]) => tag)
  }, [selectedReferences, selectedTagsByCategory, venueConditionTags])
  ...
  const tasteGroups = useMemo(() => referenceCategories.map((definition) => {
    const references = selectedReferences.filter((item) => item.reference.category === definition.label)
    const counts = references.flatMap((item) => item.reference.tags).reduce<Record<string, number>>((all, tag) => ({ ...all, [tag]: (all[tag] ?? 0) + 1 }), {})
    const requestedTags = definition.label === '웨딩홀' ? venueConditionTags : selectedTagsByCategory[definition.label] ?? []
    const photoTags = Object.entries(counts).sort((a, b) => b[1] - a[1]).map(([tag]) => tag).filter((tag) => !requestedTags.includes(tag))
    return { category: definition.label, references, requestedTags, tags: [...requestedTags, ...photoTags] }
  }).filter((group) => group.references.length || group.requestedTags.length), [selectedReferences, selectedTagsByCategory, venueConditionTags])
  const selectedConditionCount = Object.values(selectedTagsByCategory).reduce((count, tags) => count + (tags?.length ?? 0), venueConditionTags.length)
  ...
  const addAnalyzedReference = (reference: Omit<WeddingReference, 'id'>) => {
    ...
    setSelectedTagsByCategory((all) => ({ ...all, [reference.category]: reference.tags }))
    setSubmitted(false)
    setQuery('')
  ...
  return <section className="portal-subpage portal-taste-page">
    <div className="taste-workspace-actions"><Button size="sm" variant="ghost" icon={<ArrowLeft size={14} />} onClick={() => setMode('intro')}>처음으로</Button><div className="taste-mode-switch"><Button size="sm" variant={mode === 'inspire' ? 'primary' : 'ghost'} icon={<Sparkles size={14} />} onClick={() => setMode('inspire')}>둘러보기</Button><Button size="sm" variant={mode === 'search' ? 'primary' : 'ghost'} icon={<Search size={14} />} onClick={() => setMode('search')}>조건 찾기</Button></div></div>
    <div className="portal-subpage__intro"><p className="eyebrow">My wedding taste</p><h2>{mode === 'inspire' ? '마음이 가는 레퍼런스' : '조건으로 찾는 레퍼런스'}</h2><p>{mode === 'inspire' ? '좋아 보이는 이미지를 편하게 골라보세요.' : '여러 조건을 조합해 원하는 분위기를 찾아보세요.'}</p></div>

    <Card padding="none" className="taste-filter-card">
      <ReferenceCategoryTabs value={category} onChange={(next) => { setCategory(next); setQuery('') }} variant="planner" />
      {category !== '웨딩홀' && mode === 'search' && <div className="taste-search-tools"><label className="taste-search-input"><Search size={17} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder={`${category} 업체명·조건 검색`} />{(query || selectedTags.length > 0) && <button onClick={() => { setQuery(''); setSelectedTags([]) }}>초기화</button>}</label><ReferenceKeywordFilter category={category} selectedKeywords={selectedTags} onKeywordToggle={(keyword) => setSelectedTags((current) => current.includes(keyword) ? current.filter((item) => item !== keyword) : [...current, keyword])} variant="planner" /><div className="taste-upload"><div><UploadCloud size={19} /><span><strong>가지고 있는 사진으로 찾기</strong><small>AI가 분야와 해시태그를 자동으로 분석합니다.</small></span></div><Button size="sm" variant="secondary" icon={<ImagePlus size={14} />} onClick={() => setUploadOpen(true)}>이미지 분석</Button></div></div>}
    </Card>
    {category === '웨딩홀' && <VenueFilterPanel audience="client" value={venueFilters} resultCount={venueResults.length} onChange={(next) => { setVenueFilters(next); setSubmitted(false) }} />}

    <Card padding="lg" className="taste-profile-card"><header className="portal-section__head"><div><p className="eyebrow">My taste profile</p><h2>분야별 취향 해시태그</h2><span>선택한 조건과 고른 사진에서 취향을 분야별로 모았어요.</span></div><div className="taste-profile-card__counts"><Badge tone="neutral">조건 {selectedConditionCount}개</Badge><Badge tone={selections.length ? 'rose' : 'neutral'}>사진 {selections.length}장</Badge></div></header>
      {tasteGroups.length ? <><div className="taste-profile-groups">{tasteGroups.map((group) => <section key={group.category}><header><Badge tone="neutral">{group.category}</Badge><span>{group.requestedTags.length ? `조건 ${group.requestedTags.length}개` : ''}{group.requestedTags.length && group.references.length ? ' · ' : ''}{group.references.length ? `사진 ${group.references.length}장` : ''}</span></header><ReferenceTagList tags={group.tags} matchedTags={group.requestedTags} variant="planner" />{group.references.length > 0 && <div className="taste-profile-thumbnails">{group.references.map(({ reference }) => <div key={reference.id}><img src={reference.image} style={{ objectPosition: reference.imagePosition }} alt={`${reference.category} 선택 사진`} /><button type="button" aria-label="선택 삭제" onClick={() => toggleReference(reference.id)}><Trash2 size={12} /></button></div>)}</div>}</section>)}</div><footer><span>{submitted ? '전달 완료 · 조건이나 사진을 바꾸면 다시 보낼 수 있어요.' : '분야별 해시태그를 확인해 주세요.'}</span><Button icon={<Send size={15} />} onClick={send}>{existing ? '플래너에게 다시 보내기' : '플래너에게 보내기'}</Button></footer></> : <div className="taste-board-empty"><Heart size={23} /><strong>아직 발견한 취향이 없어요</strong><p>{mode === 'search' ? '필터에서 원하는 조건을 고르면 분야별 해시태그가 바로 모여요.' : '아래 사진에서 마음에 드는 장면을 고르면 분야별 해시태그가 모여요.'}</p></div>}
    </Card>

    <div className="portal-section__head taste-results-heading"><div><p className="eyebrow">References</p><h2>{category === '웨딩홀' ? '조건에 맞는 웨딩홀' : `${category} 레퍼런스`}</h2></div><div><Badge tone="neutral">{category === '웨딩홀' ? `${venueResults.length}곳 · 업로드 ${uploadedVenueReferences.length}장` : `${filtered.length}장`}</Badge><Badge tone={selections.length ? 'rose' : 'neutral'}>선택 {selections.length}</Badge></div></div>
    {category === '웨딩홀' ? <>{uploadedVenueReferences.length > 0 && <div className="taste-image-grid taste-uploaded-venue-grid">{uploadedVenueReferences.map((reference) => { const selected = selections.some((item) => item.referenceId === reference.id); return <Card padding="none" className={`taste-reference-card ${selected ? 'selected' : ''}`} key={reference.id}><button className="taste-reference-image" onClick={() => toggleReference(reference.id)}><img src={reference.image} alt="업로드한 웨딩홀 레퍼런스" /><span className="taste-select-mark">{selected ? <><Check size={14} /> 선택됨</> : <><Heart size={14} /> 선택</>}</span></button><div className="taste-reference-info"><strong>{reference.vendorName}</strong><span>{reference.purpose}</span><ReferenceTagList tags={reference.tags} max={3} variant="planner" /></div></Card> })}</div>}<div className="venue-result-grid venue-result-grid--client">{venueResults.map((venue) => { const reference = getVenuePrimaryReference(venue); const selected = selections.some((item) => item.referenceId === reference.id); return <VenueCard key={venue.id} venue={venue} audience="client" selected={selected} onToggle={() => toggleReference(reference.id)} /> })}</div></> : <div className="taste-image-grid">{filtered.map((reference) => { const selected = selections.some((item) => item.referenceId === reference.id); return <Card padding="none" className={`taste-reference-card ${selected ? 'selected' : ''}`} key={reference.id}><button className="taste-reference-image" onClick={() => toggleReference(reference.id)}><img src={reference.image} style={{ objectPosition: reference.imagePosition }} alt={`${reference.category} 취향 레퍼런스`} /><span className="taste-select-mark">{selected ? <><Check size={14} /> 선택됨</> : <><Heart size={14} /> 선택</>}</span></button><div className="taste-reference-info"><strong>{reference.vendorName}</strong><span>{reference.purpose}</span><ReferenceTagList tags={reference.tags} max={3} variant="planner" /></div></Card> })}</div>}
