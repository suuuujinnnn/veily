  ...
  return (
    <div className="portal-page">
      ...
      <main className="portal-content">
        ...
        {tab === 'estimate' && <section className="portal-subpage portal-estimate"><div className="portal-subpage__intro"><p className="eyebrow">Estimate</p><h2>계약·견적 현황</h2><p>{settings.showFullEstimate ? '계약 금액과 입금 현황을 함께 확인하세요.' : '플래너가 공개한 계약 진행 상태입니다.'}</p></div>{settings.showFullEstimate && <div className="portal-estimate-summary"><article><span>총 계약금액</span><strong>{coupleContracts.reduce((sum, item) => sum + item.totalPrice, 0).toLocaleString('ko-KR')}원</strong></article><article><span>입금 완료</span><strong>{couplePayments.filter((item) => item.status === '입금완료').reduce((sum, item) => sum + item.amount, 0).toLocaleString('ko-KR')}원</strong></article></div>}<div className="portal-contract-list">{coupleContracts.map((contract) => <article key={contract.id}><span className="portal-contract-icon"><ReceiptText size={18} /></span><div><Badge tone={contract.status === '서명완료' ? 'sage' : 'neutral'}>{contract.status}</Badge><h3>{contract.vendorName}</h3><p>{contract.productName}</p></div>{settings.showFullEstimate && <strong>{contract.totalPrice.toLocaleString('ko-KR')}원</strong>}</article>)}</div></section>}
      </main>
