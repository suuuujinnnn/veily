  ...
  return (
    <div className="portal-page">
      ...
      <main className="portal-content">
        ...
        {tab === 'calendar' && <section className="portal-subpage portal-calendar-page"><div className="portal-subpage__intro"><p className="eyebrow">Shared calendar</p><h2>공유 일정</h2><p>확정 일정과 예약 후보를 확인하고 변경합니다.</p></div><div className="availability-card"><div><span className="availability-icon"><CalendarDays size={22} /></span><div><Badge tone="amber">시간 선택 필요</Badge><h3>메이크업 테스트 가능한 시간을 알려주세요</h3><p>선택한 시간은 지윤 플래너님에게 바로 공유됩니다.</p></div></div><div className="slot-list">{slots.map((slot) => { const selected = (availability.e4 ?? []).includes(slot); return <button key={slot} onClick={() => toggleAvailability('e4', slot)} className={selected ? 'selected' : ''}><span>{selected && <Check size={13} />}</span>{slot}</button>})}</div><small>여러 시간을 선택할 수 있습니다 · 8월 7일까지 응답</small></div><VendorScheduleBoard vendor={vendors[0]} coupleId={couple.id} clientView /><div className="portal-event-list"><h3>확정된 8월 일정</h3>{coupleEvents.map((event) => <article key={event.id}><div className="portal-date"><strong>{event.date.slice(-2)}</strong><span>AUG</span></div><div><Badge tone="rose">{event.type}</Badge><h4>{event.title}</h4><p>{event.time}–{event.endTime} · {event.location}</p></div><span className="event-confirmed"><CheckCircle2 size={14} /> 확정</span></article>)}</div></section>}
