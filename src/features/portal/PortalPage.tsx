...
import { ArrowRight, CalendarDays, Check, CheckCircle2, ChevronRight, Clock3, Heart, MapPin, MessageCircle, Pause, ReceiptText, Sparkles, ThumbsUp } from 'lucide-react'
...
export function PortalPage() {
  ...
  const { couples, events, checklist, vendors, contracts, payments, budgetPlans, budgetItems, portalSettings, recommendations, availability, uploadedReferences, setRecommendation, toggleAvailability } = useDemoStore()
  ...
  const budgetPlan = budgetPlans.find((item) => item.coupleId === couple.id)
  const coupleBudgetItems = budgetItems.filter((item) => item.coupleId === couple.id)
  const plannedTotal = coupleBudgetItems.reduce((sum, item) => sum + item.plannedAmount, 0)
    ...
    <div className="portal-page">
      ...
      <main className="portal-content">
        ...
        {tab === 'estimate' && <section className="portal-subpage portal-estimate">
          ...
          <section className="portal-planned-budget">
            <header><div><p className="eyebrow">Budget plan</p><h3>계획 예산</h3></div><strong>{(budgetPlan?.targetAmount ?? plannedTotal).toLocaleString('ko-KR')}원</strong></header>
            <div className="portal-planned-budget__summary"><span>항목별 계획 합계</span><strong>{plannedTotal.toLocaleString('ko-KR')}원</strong><small>온보딩과 플래너 견적 화면에서 입력한 계획이 함께 반영됩니다.</small></div>
            <div className="portal-planned-budget__items">{coupleBudgetItems.map((item) => <article key={item.id}><div><Badge tone="neutral">{item.category}</Badge><h4>{item.title}</h4>{item.memo && <p>{item.memo}</p>}</div><strong>{item.plannedAmount.toLocaleString('ko-KR')}원</strong></article>)}{!coupleBudgetItems.length && <p className="portal-planned-budget__empty">아직 등록된 계획 예산 항목이 없습니다.</p>}</div>
          </section>
