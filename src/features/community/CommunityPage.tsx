...
export function CommunityPage() {
  ...
  const rewardEligible = view !== '질문'
  const submit = () => { setSent(true); window.setTimeout(() => { setSent(false); setComposeOpen(false); if (rewardEligible) { setRewardToast(true); window.setTimeout(() => setRewardToast(false), 2200) } }, 900) }

  return (
    <div className="page-stack community-page">
      <section className="community-hero"><div><p className="eyebrow">Planner lounge</p><h1>우리끼리 더 솔직하게,<br /><em>함께 쌓는 현장의 지혜.</em></h1><p>공개 업체 리뷰와 인증 플래너 전용 실무 대화를 한곳에서 나눕니다.</p></div>{view !== 'reviews' && <Button icon={<PenLine size={16} />} onClick={() => setComposeOpen(true)}>{rewardEligible ? '정보 공유하고 +10P' : '질문 작성'}</Button>}<div className="community-hero__seal"><ShieldCheck size={22} /><span>VERIFIED<br />PLANNERS ONLY</span></div></section>
      ...
      <Modal open={composeOpen} onClose={() => setComposeOpen(false)} eyebrow="Anonymous post" title="플래너 라운지에 글쓰기" footer={<><Button variant="ghost" onClick={() => setComposeOpen(false)}>취소</Button><Button onClick={submit}>{sent ? '등록 완료' : '익명으로 등록'}</Button></>}>
        {sent ? <div className="upload-success"><BadgeCheck size={36} /><h3>글이 등록되었어요</h3><p>{rewardEligible ? '+10P 적립을 준비하고 있어요.' : '동료 플래너에게 질문을 공유했어요.'}</p></div> : <div className="form-grid">{rewardEligible && <div className="reward-preview form-field--wide"><BadgeCheck size={18} /><div><strong>정보를 공유하고 +10P 받기</strong><span>등록 완료 후 포인트 적립 결과를 알려드려요.</span></div></div>}<label className="form-field"><span>카테고리</span><select defaultValue={view === '질문' ? '질문' : '정보 공유'}><option>정보 공유</option><option>질문</option></select></label><label className="form-field form-field--wide"><span>제목</span><input placeholder="무엇을 나누고 싶으신가요?" /></label><label className="form-field form-field--wide"><span>내용</span><textarea rows={6} placeholder="동료 플래너에게 나누고 싶은 이야기를 적어주세요." /></label></div>}
      </Modal>
      <Toast open={rewardToast} reward title="+10P 적립" message="유용한 정보를 공유해 주셔서 감사합니다." />
    </div>
  )
}
