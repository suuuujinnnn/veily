...
import type { OrderApprovalStatus, OrderRejectionReason } from '../../types'

const statusMeta: Record<OrderApprovalStatus, { label: string; group: string }> = {
  draft: { label: '발주 전', group: 'ready' }, ordered: { label: '발주 완료', group: 'ordered' }, 'reverse-pending': { label: '역발주 대기', group: 'waiting' }, approved: { label: '승인 완료', group: 'approved' }, rejected: { label: '거절', group: 'attention' }, expired: { label: '기한 초과', group: 'attention' },
}
...
export function PartnerApprovalMockPage() {
  ...
  return <div className="partner-approval-page">
    ...
    <main><section className="partner-mock-intro"><p className="eyebrow">Order approval</p><h1>예약 가능 여부 확인</h1><p>요청받은 일정과 상품을 확인한 뒤 승인 또는 거절해 주세요.</p></section>
      <div className="partner-order-list">{orderApprovals.map((order) => { const couple = couples.find((item) => item.id === order.coupleId); const vendor = vendors.find((item) => item.id === order.vendorId); const event = events.find((item) => item.id === order.relatedEventId); const meta = statusMeta[order.status]; return <Card key={order.id} className="partner-order-card">
        <div className="partner-order-card__head"><div><small>{vendor?.name}</small><h2>{order.productName}</h2></div><span className={`order-status-pill order-status-pill--${meta.group}`}>{meta.label}</span></div>
