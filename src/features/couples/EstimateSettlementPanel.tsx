...
export function EstimateSettlementPanel({ coupleId }: { coupleId: string }) {
  ...
  const settled = coupleContracts.filter((item) => item.status === '서명완료').reduce((sum, item) => sum + item.totalPrice, 0)
  ...
  return <>
    ...
    <FinanceTable
      title="분야별 계약"
      label="Contracts"
      headings={['계약일','품목','업체명','계약서','상품명','세부 구성','결제','VAT','계약총액','수수료','입금일','상태','관리']}
      rows={coupleContracts.map((item) => <tr key={item.id}><td>{item.contractDate || '-'}</td><td>{item.category}</td><td><strong>{item.vendorName}</strong></td><td>{item.contractFile || '-'}</td><td>{item.productName || '-'}</td><td className="finance-details-cell">{item.packageDetails || '-'}</td><td>{item.paymentMethod}</td><td>VAT {item.vatType}</td><td><strong>{won(item.totalPrice)}</strong></td><td>{item.commission.toLocaleString('ko-KR')}</td><td>{item.paymentDate || '-'}</td><td><Badge tone={item.status === '서명완료' ? 'sage' : item.status === '확인필요' ? 'amber' : 'neutral'}>{item.status}</Badge></td><td><RowActions onEdit={() => editContract(item)} onDelete={() => store.deleteContract(item.id)} /></td></tr>)}
      summary={<div className="finance-table-summary finance-table-summary--two"><span>총 리베이트 금액<strong>{commission.toLocaleString('ko-KR')}</strong></span><span>정산금액<strong>{won(settled)}</strong></span></div>}
    />
    <FinanceTable
      title="입금 내역"
      label="Payments"
      headings={['입금일','타입','계정','금액','비고','상태','관리']}
      rows={couplePayments.map((item) => <tr key={item.id}><td>{item.paymentDate || '-'}</td><td>{item.type === '환불' ? '환불' : '입금'}</td><td><strong>{item.account}</strong></td><td>{item.type === '환불' ? '-' : ''}{won(item.amount)}</td><td>{item.memo || '-'}</td><td><Badge tone={item.status === '입금완료' ? 'sage' : item.status === '확인필요' ? 'amber' : 'neutral'}>{item.status === '입금완료' ? '완료' : item.status === '입금예정' ? '대기' : '확인필요'}</Badge></td><td><RowActions onEdit={() => editPayment(item)} onDelete={() => store.deletePayment(item.id)} /></td></tr>)}
      summary={<div className="finance-table-summary finance-table-summary--three"><span>총계약금<strong>{contracted.toLocaleString('ko-KR')}</strong></span><span>입금<strong>{paid.toLocaleString('ko-KR')}</strong></span><span>잔금<strong>{Math.max(0, contracted - paid).toLocaleString('ko-KR')}</strong></span></div>}
    />
...
function ContractModal({ open, draft, setDraft, budgetItems, onClose, onSubmit }: { open: boolean; draft: Contract | Omit<Contract, 'id'>; setDraft: (value: Contract | Omit<Contract, 'id'>) => void; budgetItems: BudgetItem[]; onClose: () => void; onSubmit: (event: FormEvent) => void }) {
  return <Modal open={open} onClose={onClose} title={'id' in draft ? '계약 수정' : '계약 추가'} eyebrow="Contract" footer={<><Button variant="ghost" onClick={onClose}>취소</Button><Button type="submit" form="contract-form">저장</Button></>}><form id="contract-form" className="form-grid" onSubmit={onSubmit}>
    ...
    <Field label="계약일"><input type="date" value={draft.contractDate} onChange={(e) => setDraft({ ...draft, contractDate: e.target.value })} /></Field><Field label="입금일"><input type="date" value={draft.paymentDate} onChange={(e) => setDraft({ ...draft, paymentDate: e.target.value })} /></Field><Field label="총 계약금액"><input type="number" min="0" value={draft.totalPrice} onChange={(e) => setDraft({ ...draft, totalPrice: Number(e.target.value) })} /></Field>
