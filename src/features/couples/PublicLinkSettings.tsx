...
const settingItems: { key: Exclude<keyof PortalSettings, 'coupleId'>; title: string; description: string; icon: ReactNode }[] = [
  ...
  { key: 'receiveRequests', title: '요청 접수', description: '고객 포털 상단에 요청 보내기 버튼을 표시합니다.', icon: <MessageCircle size={18} /> },
...
export function PublicLinkSettings({ coupleId }: { coupleId: string }) {
  ...
  const settings = portalSettings.find((item) => item.coupleId === coupleId) ?? { coupleId, showSchedule: true, showFullEstimate: true, receiveRequests: true, showChecklist: true }
