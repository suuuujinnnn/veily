...
const statusMeta: Record<OrderApprovalStatus, { label: string; group: string }> = {
  draft: { label: '발주 전', group: 'ready' }, ordered: { label: '발주 완료', group: 'ordered' }, 'reverse-pending': { label: '역발주 대기', group: 'waiting' }, approved: { label: '승인 완료', group: 'approved' }, rejected: { label: '확인 필요', group: 'attention' }, expired: { label: '확인 필요', group: 'attention' },
}
...
export function OrderApprovalPanel({ coupleId }: { coupleId: string }) {
  ...
  const candidates = recommendations
    ...
    .filter((item) => !orders.some((order) => order.vendorId === item.vendorId && ['ordered', 'reverse-pending', 'approved'].includes(order.status)))
  ...
  return <div className="order-approval-workspace">
    ...
    <div className="order-list">{orders.map((order) => { const vendor = vendors.find((item) => item.id === order.vendorId); const event = events.find((item) => item.id === order.relatedEventId); const meta = statusMeta[order.status]; return <Card className={`order-card order-card--${order.status}`} key={order.id}>
      <div className="order-card__head"><div><span><Building2 size={17} /></span><div><small>{vendor?.category ?? '제휴업체'}</small><h3>{vendor?.name ?? '업체 정보 확인 중'}</h3></div></div><span className={`order-status-pill order-status-pill--${meta.group}`}>{meta.label}</span></div>
      ...
      {order.status === 'reverse-pending' && <p className="order-result"><CalendarClock size={16} /> 업체의 역발주 승인을 기다리고 있습니다.</p>}
    </Card>})}</div>
  </div>
}
