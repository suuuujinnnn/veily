...
import { CalendarDays, CalendarPlus, CheckCircle2, ChevronRight, Heart, MessageCircle, MessageSquareText, Plus, Target, WalletCards } from 'lucide-react'
...
export function ConsultationsPanel({ coupleId, embedded = false }: { coupleId: string; embedded?: boolean }) {
  ...
  return <div className={`consultation-workspace ${embedded ? 'consultation-workspace--embedded' : ''}`}>
    <div className="feature-panel-heading"><div><h2>상담</h2><p>고객의 기본 조건과 상담 기록을 간결하게 관리합니다.</p></div><div className="heading-actions"><Link to={`/consultation/${coupleId}`}><Button variant="secondary" icon={<Heart size={16} />}>{preferenceCard ? '상담 카드 수정' : '상담 카드 만들기'}</Button></Link><Button icon={<Plus size={16} />} onClick={() => setOpen(true)}>상담 추가</Button></div></div>

    {preferenceCard ? <Card padding="none" className="couple-consultation-card couple-consultation-card--compact">
      <header className="couple-consultation-card__header">
        <div><span className="document-icon"><Heart size={18} /></span><div><h3>상담 카드</h3><small>{preferenceCard.createdAt} 업데이트</small></div></div>
        <div><Badge tone="sage">{preferenceCard.source}</Badge><Link to={`/consultation/${coupleId}`}>수정 <ChevronRight size={14} /></Link></div>
      </header>
