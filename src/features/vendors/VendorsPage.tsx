  ...
  return (
    <div className="page-stack vendors-page">
      ...
      <section className="vendor-browser">
        ...
        <div className="vendor-grid">{filtered.map((vendor) => { const isRecommended = recommendations.some((item) => item.coupleId === 'c1' && item.vendorId === vendor.id); return <Card padding="none" className="vendor-card" key={vendor.id}><div className="vendor-card__image"><img src={vendor.image} style={{ objectPosition: vendor.imagePosition }} alt={`${vendor.name} 이미지`} /><Badge tone="dark"><Sparkles size={11} /> {vendor.match}% MATCH</Badge><Link to={`/vendors/${vendor.id}`} aria-label="이미지 더 보기"><ImagePlus size={16} /></Link></div><div className="vendor-card__body"><div className="vendor-card__top"><span>{vendor.category} · {vendor.location}</span><strong>{vendor.priceRange}</strong></div><h3>{vendor.name}</h3><p>{vendor.summary}</p><div className="tag-row">{vendor.tags.map((tag) => <span key={tag}>{tag}</span>)}</div><div className="vendor-card__actions"><Link to={`/vendors/${vendor.id}`}>상세 보기 <ChevronRight size={13} /></Link><Button size="sm" variant={isRecommended ? 'secondary' : 'primary'} onClick={() => recommend(vendor.id)}>{isRecommended ? '추천에 담김' : '서윤 & 도현에게 추천'}</Button></div></div></Card>})}</div>
      </section>
    </div>
  )
}
