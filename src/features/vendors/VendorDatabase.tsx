...
import { Badge, Button, Card, Modal, SegmentedTabs } from '../../components/ui'
...
export function VendorDatabase({ vendorIds }: { vendorIds: string[] }) {
  ...
  const selectCatalog = (id: string) => {
    ...
    if (id === 'all') return
    const group = vendorCatalogGroups.find((item) => item.id === id)
    if (group) { setTargetCatalogId(group.id); setRenameValue(group.name) }
  }

  return <section className="vendor-db">
    <section className="vendor-catalogs"><div className="vendor-catalogs__head"><div><p className="eyebrow">My catalog</p><h3>나만의 업체 카탈로그</h3><p>투어 후보나 콘셉트별로 업체를 묶어 위시리스트처럼 관리하세요.</p></div><Button variant="secondary" size="sm" icon={<FolderPlus size={15} />} onClick={() => setCatalogOpen(true)}>그룹 만들기</Button></div><SegmentedTabs className="vendor-catalog-switch" value={activeCatalogId} onChange={selectCatalog} ariaLabel="업체 카탈로그" scrollable items={[{ value: 'all', label: '전체 업체', count: vendors.length }, ...vendorCatalogGroups.map((group) => ({ value: group.id, label: group.name, icon: <FolderHeart size={13} />, count: group.vendorIds.length }))]} />{activeCatalog && <div className="vendor-catalogs__rename"><label><span>그룹명</span><input value={renameValue || activeCatalog.name} onChange={(event) => setRenameValue(event.target.value)} /></label><Button size="sm" variant="ghost" onClick={() => { const name = (renameValue || activeCatalog.name).trim(); if (name) renameVendorCatalogGroup(activeCatalog.id, name) }}>이름 저장</Button></div>}<div className="vendor-catalogs__target"><span>카드에서 담을 카탈로그</span><select value={targetCatalogId} onChange={(event) => setTargetCatalogId(event.target.value)}>{vendorCatalogGroups.map((group) => <option value={group.id} key={group.id}>{group.name}</option>)}</select></div></section>
    <div className="vendor-db__summary"><div><Building2 size={18} /><span>검색 결과</span><strong>{filtered.length}</strong></div><Button size="sm" icon={<Plus size={14} />} onClick={() => edit()}>업체 추가</Button></div>
