...
import { Building2, CalendarClock, Camera as Instagram, Edit3, FolderHeart, FolderPlus, MapPin, Plus, Search } from 'lucide-react'
...
import { isVendorStale, vendorOperationalText } from './vendorInfoUtils'
...
export function VendorDatabase() {
  ...
  const filtered = useMemo(() => vendors.filter((vendor) => {
    ...
    return (category === '전체' || vendor.category === category) && (!query || haystack.includes(query.toLowerCase())) && (!tag || vendor.tags.some((item) => item.includes(tag))) && (!catalog || catalog.vendorIds.includes(vendor.id))
  }), [activeCatalogId, category, query, tag, vendorCatalogGroups, vendors])
