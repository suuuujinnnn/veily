...
export function AddEventModal({ open, onClose, onAdded, initialDate, initialCoupleId }: AddEventModalProps) {
  ...
  useEffect(() => { if (open) { if (initialDate) setDate(initialDate); if (initialCoupleId) { setCoupleId(initialCoupleId); setVisibility('couple-shared') } } }, [initialCoupleId, initialDate, open])
  ...
  const submit = () => {
    ...
    addEvent({ ...(visibility === 'couple-shared' ? { coupleId, workflowType: template?.label, travelMode } : {}), visibility, type, title, date, time, endTime, location: location || '장소 미정', durationMinutes: duration, memo })
  ...
  return <Modal open={open} onClose={onClose} eyebrow="New schedule" title="새 일정 등록" footer={<><Button variant="ghost" onClick={onClose}>취소</Button><Button icon={<CalendarPlus size={15} />} onClick={submit} disabled={!title.trim()}>일정 등록</Button></>}>
    <div className="form-grid">
      ...
      {visibility === 'couple-shared' && <label className="form-field"><span>상위 일정 유형</span><select value={type} onChange={(event) => setType(event.target.value as EventType)}>{['미팅','드레스','스튜디오','메이크업','계약','본식'].map((item) => <option key={item}>{item}</option>)}</select></label>}
